...
void DmaDataUpdateThread()
{
	while (true)
	{
		try {
			...
			LOG_CRITICAL("Exception in DMA update thread: %s", e.what());
			Sleep(1000);
		}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    try {
        ...
        // Launch DMA data update thread
        LOG_INFO("Launching DMA data update thread...");
        std::thread(DmaDataUpdateThread).detach();
        ...
        while (msg.message != WM_QUIT) {
            try {
                ...
                // Draw ESP elements (only if environment is initialized by DMA thread)
                if (EnvironmentInstance != nullptr) {
                    DrawSpectators();
                    DrawRadar();
                    DrawOtherEsp();
