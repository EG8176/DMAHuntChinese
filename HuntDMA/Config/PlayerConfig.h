...
public:
    ...
    bool ShowDead = false;
    int BoxType = 1; // 0=None, 1=Corner, 2=Full
    bool DrawHead = true;
    ...
    json ToJson()
    {
        ...
        j[ConfigName][LIT("BoxType")] = BoxType;
        j[ConfigName][LIT("DrawHead")] = DrawHead;
        ...
        ToJsonColor(&j, LIT("BonesColor"), &BonesColor);
        ToJsonColor(&j, LIT("FriendBonesColor"), &FriendBonesColor);
    ...
    void FromJson(const json& j)
    {
        ...
        if (j[ConfigName].contains(LIT("BoxType")))
            BoxType = j[ConfigName][LIT("BoxType")];
        else if (j[ConfigName].contains(LIT("DrawFrames"))) {
            // Backward compat: old bool DrawFrames -> new int BoxType
            bool old = j[ConfigName][LIT("DrawFrames")];
            BoxType = old ? 1 : 0;
        }
        if (j[ConfigName].contains(LIT("DrawHead")))
            DrawHead = j[ConfigName][LIT("DrawHead")];
        else if (j[ConfigName].contains(LIT("DrawHeadInFrames")))
            DrawHead = j[ConfigName][LIT("DrawHeadInFrames")];
        ...
        FromJsonColor(j, LIT("BonesColor"), &BonesColor);
        FromJsonColor(j, LIT("FriendBonesColor"), &FriendBonesColor);
        if (j[ConfigName].contains(LIT("DrawBones")))
